...
class ResumeRepository:
    ...
    def get_match(self, stack, stack_need) -> float:
        return len(
            (
                set([s.strip().lower() for s in stack_need])
                & set([s.strip().lower() for s in stack])
            )
        ) / len(stack_need)
    ...
    async def search(
        ...
        resumes = [
            SearchResume(**document)
            async for document in self.collection.find(filter_).skip(skip).limit(limit)
        ]
...
class UploadedResumeRepository:
    ...
    async def create(self, user_id: int, resumes: List[str]) -> UploadedResume:
        ur = UploadedResume(user_id=user_id, resumes=resumes)
    ...
    async def get_by_id(self, object_id: str) -> Optional[object_model]:
        ...
        resumes = [
            _
            async for _ in mongo_db.Resumes.find(
